...
var clerkClient = new Proxy(clerkClientSingleton, {
  get(_target, property) {
    ...
    const client = (0, import_backend.createClerkClient)({ ...env, userAgent: `${"@clerk/express"}@${"2.1.17"}` });
    if (env.secretKey) {
...
var authenticateRequest = (opts) => {
  ...
  const isSatellite = (0, import_utils2.handleValueOrFn)(isSatelliteInput, clerkRequest.clerkUrl, env.isSatellite);
  const domain = (0, import_utils2.handleValueOrFn)(domainInput, clerkRequest.clerkUrl) || env.domain;
  const signInUrl = signInUrlInput || env.signInUrl;
  const proxyUrl = absoluteProxyUrl(
    (0, import_utils2.handleValueOrFn)(proxyUrlInput, clerkRequest.clerkUrl, env.proxyUrl),
...
var resolveDefaultClerkClient = (options) => {
  ...
  const env = { ...loadApiEnv(), ...loadClientEnv() };
  return (0, import_backend2.createClerkClient)({
    ...env,
    ...options.apiUrl ? { apiUrl: options.apiUrl } : {},
    ...options.apiVersion ? { apiVersion: options.apiVersion } : {},
    userAgent: `${"@clerk/express"}@${"2.1.17"}`
  });
};
